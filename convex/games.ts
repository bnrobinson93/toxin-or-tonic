...
function calculateSpeedBonus(timeMs: number, max: number): number {
  ...
  const fraction =
    1 -
    (timeMs - SPEED_BONUS_FULL_MS) /
      (SPEED_BONUS_ZERO_MS - SPEED_BONUS_FULL_MS);
...
export const submitAnswer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (diff === "hard") {
      ...
      const correctCount =
        args.hardSubQuestionsCorrect ?? (primaryCorrect ? cfg.subQuestions : 0);
      roundScore += cfg.primaryPerSubQ * correctCount;
      if (bonusCorrect) roundScore += cfg.bonus;
      if (correctCount > 0)
        roundScore += calculateSpeedBonus(args.timeToAnswerMs, cfg.speedMax);
    } else {
    ...
    if (isLastRound) {
      await ctx.db.insert("leaderboardEntries", {
        ...
        displayName: session.isAnonymous
          ? "Anonymous Forager"
          : session.playerId,
    ...
    return {
      ...
      plant: {
        ...
        identificationTips: plant.identificationTips,
        parsedMedicinalInfo: plant.parsedMedicinalInfo,
        parsedToxicityInfo: plant.parsedToxicityInfo,
        parsedEdibilityInfo: plant.parsedEdibilityInfo,
...
function validatePrimaryAnswer(
  ...
  if (difficulty === "easy") {
    ...
    const normalizedAnswer =
      answer.toLowerCase() === "edible" ? "neutral" : answer.toLowerCase();
    const normalizedCategory =
      plant.category.toLowerCase() === "edible"
        ? "neutral"
...
function validateBonusAnswer(
  ...
  return allDetails.some(
    (detail) =>
      detail.includes(answer.toLowerCase()) ||
      answer.toLowerCase().includes(detail),
  );
}
